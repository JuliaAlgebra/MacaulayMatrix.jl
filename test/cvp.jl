...
function test_elim(T = Float64)
    ...
    x = Macaulay.eliminate_indices(M, [3], 2)
    @test x / x[1] ≈ [1, -1, 0] rtol = 1e-6
    x = Macaulay.eliminate_indices(M, [3], 2, solver = Macaulay.Manopt.LevenbergMarquardt)
    @test x / x[1] ≈ [1, -1, 0] rtol = 1e-6
    M = T[
        1 0 1 0
        0 1 0 1
    ...
    x = Macaulay.eliminate_indices(M, [3, 4], 2; solver = Macaulay.Manopt.LevenbergMarquardt)
...
function test_cvp(T = Float64)
    ...
    @testset "$ellipsoid" for ellipsoid in [true, false]
        ...
        if ellipsoid
            ...
            push!(solvers, Macaulay.Manopt.GradientDescentState)
        else
            ...
            push!(solvers, Macaulay.Manopt.LevenbergMarquardt)
        end
        rank_checks = Any[nothing]
        if !ellipsoid
            push!(rank_checks, Macaulay.MM.LeadingRelativeRankTol(Base.rtoldefault(Float64)))
        end
        @testset "$(string(solver)[1:8])" for solver in solvers
            @testset "$(string(rank_check)[1:4])" for rank_check in rank_checks
                @testset "$α" for α in [zero(T), one(T), T(2)]
                    p = x^2 - 2x + α
                    c, ε = Macaulay.cvp(p, 2; ellipsoid, solver, rank_check)
